  ...
  <Split v-model="split" :min="0.1" :max="0.5" class="split" @on-move-end="moveEnd">
    ...
    <div slot="right" class="split-pane">
      <common-table v-model="tableData" :columns="columns" :total="total" :show="showTable" :showSearch="false"
                    @changePage="changePage" @changePageSize="changePageSize" @onRowClick="onRowClick" :page="page">
        <div slot="operate">
...
<script>
  ...
  export default {
    ...
    methods: {
      getList(mdtype, mdid) {
        if(mdtype) this.mdtype = mdtype;
        if(mdid) this.mdid = mdid;
        this.axios.request({
          url: '/tenant/basedata/vehiclemodel/list',
          method: 'post',
          data: {
            KEYWORD: this.KEYWORD,
            mdtype: this.mdtype == 0 ? '' : this.mdtype,
            mdid: this.mdid,
            page: this.page,
            limit: this.limit,
      ...
      select(data) {
        if(Object.prototype.toString.call(data) === "[object Array]"){
          if(data.length > 0){
            this.getList(data[0].type || '0',data[0].id + '');
          }else{
            this.getList();
          }
        }
