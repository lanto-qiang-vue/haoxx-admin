          ...
          <Panel name="1">
            ...
            <Form slot="content" :model="formData" ref="list" :rules="rules" :label-width="120" class="common-form">
              ...
              <FormItem label="备注描述:" style="width:100%;">
                <Input type="textarea" style="width: 100%;" v-model="formData.REMARK" placeholder="请输入备注信息..."> </Input>
              </FormItem>
            </Form>
          </Panel>
          <Panel name="2">
...
<script>
  ...
  export default {
    name: 'service-combo',
    components: {commonTable, selectItem, ModalTitle,},
    mixins: [mixin],
    data() {
      return {
        value1: [1, 2, 3],
    ...
    methods: {
      ...
      tosave() {
        ...
        if (this.IS_PREINSTALL) {
          this.formData.IS_PREINSTALL = this.isorno[0].code;
        } else {
          this.formData.IS_PREINSTALL = this.isorno[1].code;
        }
        this.axios.request({
